...
# #
# results=torch.load("frist_pos.pt")[:,None]
#
#
# trajs=torch.zeros([1,1, 2], dtype=torch.float32)
# for i in range(len(results)):
#     to_add=results[i][None]
#     if not ( (  (trajs[:,:,:2] - to_add[:,:,:2]).abs().sum(-1).sum(-1) < 0.25 ).any()):
...
# #
# #
# torch.save(trajs,"fileter_pos.pt")
#0.25  953173 204801 0.21486213430076775 0.5532276103918264

trajs=torch.load("fileter_pos.pt")[1:]

tokenize_center = Kdisk_cluster(X=trajs, N=2048, tol=2)

tokenize_center=tokenize_center[:,0]

with open("sim_first2048.pkl", "wb") as f:
