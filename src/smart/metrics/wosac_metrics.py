...
class WOSACMetrics(Metric):
    ...
    def update(
        ...
        if  (('keguo' in working_dir) or ("guoke" in working_dir)) :#and ('keguo_projects' not in working_dir) :#and self.challenge_type==_ChallengeType.SIM_AGENTS:#Sor "guoke" in working_diros.environ.get("CUDA_VISIBLE_DEVICES", "") in ["", "0"] and
