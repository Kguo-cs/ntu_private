...
from src.smart.tokens.attr_tokenizer import Attr_Tokenizer
...
class InfGenAgentDecoder(nn.Module):
    ...
    def transform_rel(self, token_traj, prev_pos, prev_heading=None):
        ...
        rot_mat[:, :, 1, 0] = sin
        rot_mat[:, :, 1, 1] = cos
        agent_diff_rel = torch.bmm(token_traj.view(-1, traj_num, 2), rot_mat.view(-1, 2, 2)).view(num_agent, num_step,
                                                                                                  traj_num, traj_dim)
    ...
    def _agent_token_embedding(self, data, agent_token_index, agent_state, agent_offset_token_idx, pos_a, head_a,
        ...
        agent_token_emb_veh = torch.cat(
            [agent_token_emb_veh, self.bos_token_emb(torch.zeros(1, device=pos_a.device).long())])
        agent_token_emb_ped = torch.cat(
            [agent_token_emb_ped, self.bos_token_emb(torch.zeros(1, device=pos_a.device).long())])
        agent_token_emb_cyc = torch.cat(
            [agent_token_emb_cyc, self.bos_token_emb(torch.zeros(1, device=pos_a.device).long())])

        # add invalid token embedding
        agent_token_emb_veh = torch.cat(
            [agent_token_emb_veh, self.no_token_emb(torch.zeros(1, device=pos_a.device).long())])
        agent_token_emb_ped = torch.cat(
            [agent_token_emb_ped, self.no_token_emb(torch.zeros(1, device=pos_a.device).long())])
        agent_token_emb_cyc = torch.cat(
            [agent_token_emb_cyc, self.no_token_emb(torch.zeros(1, device=pos_a.device).long())])
        ...
        self.grid_token_emb = torch.cat(
            [self.grid_token_emb, self.invalid_offset_token_emb(torch.zeros(1, device=pos_a.device).long())])
        ...
        agent_shapes = data['agent']['shape'].clone()[filter_mask].repeat_interleave(
    ...
    def _build_interaction_edge(self, data, pos_a, head_a, state_a, head_vector_a, batch_s, mask, pad_mask=None,
                                inference_mask=None,
        ...
        if is_training:
            ...
            edge_index_seed2a, r_seed2a = self._build_a2sa_edge(data, pos_a, head_a, head_vector_a, batch_s,
                                                                mask_a.clone(), ~pad_mask.clone(),
                                                                inference_mask=inference_mask,
                                                                r=self.pl2seed_radius, max_num_neighbors=300,
                                                                seq_mask=seq_mask, seq_index=seq_index,
                                                                grid_index_a=grid_index_a, mode='insert')
    ...
    def _build_map2agent_edge(self, data, pos_a, head_a, state_a, head_vector_a, batch_s, batch_pl,
        ...
        # add the edges which connect seed agents
        if is_training:
            edge_index_pl2seed, r_pl2seed = self._build_map2sa_edge(data, pos_a, head_a, head_vector_a, batch_s,
                                                                    batch_pl,
                                                                    ~pad_mask.clone(), inference_mask=inference_mask,
                                                                    r=self.pl2seed_radius, max_num_neighbors=2048,
                                                                    mode='insert')
    ...
    def _build_a2sa_edge(self, data, pos_a, head_a, head_vector_a, batch_s, mask_a, mask_sa,
        ...
        edge_index_a2sa = radius(x=pos_s[:, :2], y=pos_s[mask_s_sa, :2], r=r,
                                 batch_x=batch_s, batch_y=batch_s[mask_s_sa],
                                 max_num_neighbors=max_num_neighbors)  # x is src,  y is dst     [dst,src]
        edge_index_a2sa = edge_index_a2sa[[1, 0]]  # src,dst
        edge_index_a2sa = edge_index_a2sa[:,
                          ~mask_s_sa[edge_index_a2sa[0]] & mask_s[edge_index_a2sa[0]]]  # src exist and not sa
    ...
    def _build_map2sa_edge(self, data, pos_a, head_a, head_vector_a, batch_s, batch_pl,
                           mask_sa, inference_mask=None, r=None, max_num_neighbors=32,
    ...
    def get_inputs(self, data: HeteroData) -> Dict[str, torch.Tensor]:
        ...
        next_token_eval_mask = next_token_eval_mask * next_token_eval_mask.roll(shifts=-1,
        ...
        next_state_eval_mask = next_state_eval_mask * next_state_eval_mask.roll(shifts=-1,
                                                                                dims=1) * next_state_eval_mask.roll(
            shifts=1, dims=1)
        for bos_token_index_ in bos_token_index:
    ...
    def _build_seq(self, device, data, num_agent, num_step, av_index, sort_indices):
        ...
        if self.seed_attn_to_av:  # each seed mask is
            seq_mask[..., av_index] = True
        seq_mask = seq_mask.transpose(0, 1).reshape(-1, num_agent + num_graph * self.num_seed_feature)

        seq_index = torch.cat([torch.zeros(num_agent), (torch.arange(self.num_seed_feature) + 1).repeat(num_graph)]).to(
            device)
        seq_index = seq_index[:, None].repeat(1, num_step)
        # 0, 0, 0, ..., 1, 2, 3, ...
        for b in range(num_graph):
    ...
    def _build_occ_gt(self, data, seq_mask, pos_rel_index_gt, pos_rel_index_gt_seed=None, mask_seed=None,
        ...
        data['agent']['agent_occ'] = torch.zeros(data.num_graphs * self.num_seed_feature, num_step,
        ...
        if mode == 'edge_index':
            ...
            for src_index in torch.unique(edge_index[1]):  # 10*18 src,
                # decode src
            ...
            seq_mask = seq_mask.reshape(num_step, self.num_seed_feature, -1).transpose(0, 1)[...,
    ...
    def forward(self,
        ...
        agent_shape = data['agent']['shape'].clone()  # (a, 3)
        ...
        bos_index = torch.where(is_bos.any(dim=1), torch.argmax(is_bos.long(), dim=1), torch.tensor(0))
        eos_index = torch.where(is_eos.any(dim=1), torch.argmax(is_eos.long(), dim=1),
                                torch.tensor(num_step - 1))  # not `-1`
        ...
        pos_a_p, head_a_p, state_a_p, head_vector_a_p, grid_index_a_p, pad_mask = \
            self._pad_feat(data.num_graphs, av_index, pos_a, head_a, agent_state_index, head_vector_a,
                           agent_grid_token_idx)
        edge_index_t, r_t = self._build_temporal_edge(data, pos_a_p, head_a_p, state_a_p, head_vector_a_p,
                                                      temporal_mask)

        # placeholder for seed agent
        batch_s = torch.cat([
            torch.cat([data['agent']['batch'], torch.arange(data.num_graphs, device=device
                                                            ).repeat_interleave(repeats=self.num_seed_feature, dim=0)],
                      dim=0)
        ...
        edge_index_a2a, r_a2a, (na2a, na2sa) = self._build_interaction_edge(data, pos_a_p, head_a_p, state_a_p,
                                                                            head_vector_a_p, batch_s,
                                                                            interact_mask, pad_mask=pad_mask,
                                                                            av_index=av_index,
                                                                            seq_mask=seq_mask, seq_index=seq_index,
                                                                            grid_index_a=grid_index_a_p, **plot_kwargs)

        edge_index_pl2a, r_pl2a, (npl2a, npl2sa) = self._build_map2agent_edge(data, pos_a_p, head_a_p, state_a_p,
                                                                              head_vector_a_p, batch_s, batch_pl,
                                                                              interact_mask, pad_mask=pad_mask,
                                                                              av_index=av_index)
        ...
        if self.use_grid_token:
            self._build_occ_gt(data, seq_mask, agent_grid_token_idx.long(), edge_index=edge_index_a2a[:, -na2sa:],
                               mode='edge_index')
        ...
        if self.use_grid_token:
            occ_embed_a = self.seed_agent_occ_embed(
                grid_agent_occ_gt_seed.transpose(0, 1).reshape(-1, self.grid_size).float())
        ...
        state_sa[mask_sa] = self.enter_state  # random enter state
        ...
        if self.use_head_token:
            ...
            next_head_rel_prob_seed = self.seed_heading_rel_token_predict_head(feat_sa)  # predict heading
        ...
        if self.use_grid_token:
            next_offset_xy_seed = self.seed_offset_xy_predict_head(feat_sa)  # predict next offest xy
        ...
        next_state_index_gt_seed = torch.concat([next_state_index_gt_seed,
                                                 torch.zeros(
                                                     (next_state_prob_seed.shape[0] - next_state_index_gt_seed.shape[0],
                                                      next_state_index_gt_seed.shape[1]), device=device)]).long()
        seed_enter_mask = next_state_index_gt_seed == self.enter_state
        next_state_index_gt_seed = torch.full(next_state_index_gt_seed.shape, self.seed_state_type.index('invalid'),
                                              device=device)
    ...
    def inference(self,
        ...
        filter_mask = data['agent']['state_idx'][:,
                      (self.num_historical_steps - 1) // self.shift - 1] != self.invalid_state

        seed_step_mask = data['agent']['state_idx'][:,
                         (self.num_historical_steps - 1) // self.shift:] == self.enter_state
        ...
        for t in (
            ...
            valid_state_mask = state_a[:, (
                                                      self.num_historical_steps - 1) // self.shift - 1 + t] != self.invalid_state  # TODO: only support bs=1
            distance = ((pos_a[:, (self.num_historical_steps - 1) // self.shift - 1 + t, :2] - pos_a[av_index, (
            ...
            while True:
                ...
                assert torch.all(state_a[:, :(self.num_historical_steps - 1) // self.shift + t][
                                     ~interact_mask[:,
                                      :(self.num_historical_steps - 1) // self.shift + t]] == self.invalid_state) and \
                       torch.all(state_a[:, :(self.num_historical_steps - 1) // self.shift + t][
                                     interact_mask[:,
                                     :(self.num_historical_steps - 1) // self.shift + t]] != self.invalid_state), \
                ...
                if feat_a.shape[1] != feat_seed.shape[1]:
                    ...
                    feat_a = torch.cat([feat_a, feat_a[:, -1:].repeat(1, feat_seed.shape[1] - feat_a.shape[1], 1)],
                                       dim=1)
                ...
                # 0, 0, 0, ..., N+1, N+2, ...
                seq_index = torch.cat(
                    [torch.zeros(pos_a.shape[0] - num_new_agents), torch.arange(num_new_agents + 1) + 1]).to(device)
                # 0, 2, 1, ..., N+1, N+2, ...
                ...
                edge_index_pl2seed, r_pl2seed = self._build_map2sa_edge(data, pos_a_p, head_a_p, head_vector_a_p,
                                                                        batch_s, batch_pl,
                                                                        mask_sa=~pad_mask.clone(),
                                                                        inference_mask=inference_mask_sa,
                                                                        r=self.pl2seed_radius,
                ...
                if self.use_grid_token:
                    ...
                    occ_embed_a = self.seed_agent_occ_embed(
                        grid_agent_occ_gt_t_1.reshape(1, self.grid_size).float()).repeat(num_seed_feature, 1)
                    edge_index_occ2sa_src = torch.arange(feat_a.shape[0] * feat_a.shape[1], device=device).long()
                    edge_index_occ2sa_src = edge_index_occ2sa_src[
                        (~pad_mask.transpose(0, 1).reshape(-1)) & (inference_mask_sa.transpose(0, 1).reshape(-1))]
                    edge_index_occ2sa_tgt = torch.arange(occ_embed_a.shape[0], device=device).long()
                    edge_index_occ2sa = torch.stack([edge_index_occ2sa_tgt, edge_index_occ2sa_src], dim=0)
                ...
                for i in range(self.seed_layers):
                    ...
                    feat_a = self.pt2sa_attn_layers[i]((
                        map_enc['pt_token'].repeat_interleave(repeats=num_infer_step, dim=0).reshape(-1, num_infer_step,
                ...
                if self.predict_occ:
                    grid_agent_occ_seed = self.grid_agent_occ_head(
                        feat_seed[:, (self.num_historical_steps - 1) // self.shift - 1 + t])  # (num_seed, grid_size)
                ...
                next_state_prob_seed = self.seed_state_predict_head(
                    feat_seed[:, (self.num_historical_steps - 1) // self.shift - 1 + t])
                next_state_idx_seed = next_state_prob_seed.softmax(dim=-1).argmax(dim=-1, keepdim=True)  # +1
                ...
                if self.use_grid_token:
                    ...
                    next_pos_seed = self.attr_tokenizer.decode_pos(next_pos_rel_idx_seed[..., 0], y=ego_pos_t_1,
                                                                   theta_y=ego_head_t_1)
                    if self.inference_filter_overlap:
                        if grid_agent_occ_gt_t_1[
                            next_pos_rel_idx_seed[..., 0]]:  # TODO: only support insert num=1 for each iter!!!
                ...
                pred_shape = torch.cat(
                    [pred_shape, new_shape_a[:, (self.num_historical_steps - 1) // self.shift - 1 + t]])

                new_state_a[:,
                (self.num_historical_steps - 1) // self.shift - 1 + t] = next_state_idx_seed  # all enter state
                ...
                new_agent_token_emb = self.no_token_emb(torch.zeros(1, device=device).long())[None, :].repeat(
                    num_new_agent, num_infer_step, 1)
                new_agent_token_emb[:, (self.num_historical_steps - 1) // self.shift - 1 + t] = self.bos_token_emb(
                ...
                new_agent_token_traj_all = torch.zeros((num_new_agent, self.token_size, self.shift + 1, 4, 2),
                                                       device=device)
                ...
                new_categorical_embs = [self.type_a_emb(new_type_a.reshape(-1).long()),
                ...
                new_labels[(
                                       self.num_historical_steps - 1) // self.shift + t] = f'A{num_new_agents}'  # the first step after bos step!
                ...
                # sanity check
                assert torch.all(motion_vector_sa[:, :(
                                                                  self.num_historical_steps - 1) // self.shift - 1 + t] == self.invalid_motion_value) and \
                       torch.all(motion_vector_sa[:,
                                 (self.num_historical_steps - 1) // self.shift - 1 + t] == self.motion_gap), \
                ...
                assert pos_a.shape[0] == head_a.shape[0] == head_vector_a.shape[0] == interact_mask.shape[0] == \
                       pad_mask.shape[0] == inference_mask_sa.shape[0] == (
                                   num_agent + num_new_agent), f"Inconsistent shapes!"
                ...
                edge_index_a2sa, r_a2sa = self._build_a2sa_edge(data, pos_a, head_a, head_vector_a, batch_s,
                                                                ...
                                                                **plot_kwargs)
                edge_index_pl2sa, r_pl2sa = self._build_map2sa_edge(data, pos_a, head_a, head_vector_a, batch_s,
                                                                    batch_pl,
                                                                    mask_sa=~pad_mask.clone(),
                                                                    inference_mask=inference_mask_sa,
                                                                    r=self.pl2sa_radius,
                ...
                if self.use_head_token:
                    next_head_rel_prob_seed = self.seed_heading_rel_token_predict_head(
                        feat_a[-num_new_agent:, (self.num_historical_steps - 1) // self.shift - 1 + t])
                    next_head_rel_idx_seed = next_head_rel_prob_seed.softmax(dim=-1).argmax(dim=-1, keepdim=True)
                    next_head_seed = wrap_angle(
                        self.attr_tokenizer.decode_heading(next_head_rel_idx_seed) + ego_head_t_1)
                else:
                ...
                # first predict position idx, then predict heading and position offset
            ...
            next_state_idx_list[-1] = torch.cat(
                [next_state_idx_list[-1], torch.full((num_new_agents, 1), self.enter_state, device=device).long()])
            ...
            edge_index_t, r_t = self._build_temporal_edge(data, pos_a, head_a, state_a, head_vector_a, temporal_mask,
            ...
            edge_index_a2a, r_a2a = self._build_interaction_edge(data, pos_a, head_a, state_a, head_vector_a, batch_s,
                                                                 interact_mask, inference_mask=inference_mask,
                                                                 av_index=av_index, **plot_kwargs)
            edge_index_pl2a, r_pl2a = self._build_map2agent_edge(data, pos_a, head_a, state_a, head_vector_a, batch_s,
                                                                 batch_pl,
                                                                 interact_mask, inference_mask=inference_mask,
                                                                 av_index=av_index, **plot_kwargs)
            ...
            for i in range(self.num_layers):
                ...
                feat_a = feat_a.reshape(-1, num_infer_step, self.hidden_dim).transpose(0, 1).reshape(-1,
                                                                                                     self.hidden_dim)
                feat_a = self.pt2a_attn_layers[i]((
                    map_enc['pt_token'].repeat_interleave(repeats=num_infer_step, dim=0).reshape(-1, num_infer_step,
                                                                                             self.hidden_dim).transpose(
                ...
                if t == 0:
                    ...
                    feat_a_t_dict[i + 1][:n, (self.num_historical_steps - 1) // self.shift - 1 + t] = feat_a[:n, (
                                                                                                                             self.num_historical_steps - 1) // self.shift - 1 + t]
            ...
            next_token_prob_softmax = torch.softmax(next_token_prob, dim=-1)
            topk_token_prob, next_token_idx = torch.topk(next_token_prob_softmax, k=self.motion_beam_size,
                                                         dim=-1)  # both (num_agent, beam_size) e.g. (31, 5)
            ...
            next_token_traj = torch.gather(agent_token_traj_all, 1,
                                           expanded_token_index)  # (num_agent, beam_size, 6, 4, 2)
            ...
            agent_diff_rel = torch.bmm(next_token_traj.view(-1, 4, 2),
                                       rot_mat[:, None, None, ...].repeat(1, self.motion_beam_size, self.shift + 1, 1,
                                                                          1).view(
                                           -1, 2, 2)).view(num_agent, self.motion_beam_size, self.shift + 1, 4, 2)
            agent_pred_rel = agent_diff_rel + pos_a[:, None, None, None,
                                              (self.num_historical_steps - 1) // self.shift - 1 + t, :]
            ...
            mask[is_invalid, (
                                         self.num_historical_steps - 1) // self.shift + t] = False  # to handle those newly-added agents
            interact_mask[is_invalid, (self.num_historical_steps - 1) // self.shift + t] = False

            agent_token_emb[is_invalid, (self.num_historical_steps - 1) // self.shift + t] = self.no_token_emb(
                torch.zeros(1, device=device).long())
            ...
            type_emb[is_invalid, (self.num_historical_steps - 1) // self.shift + t] = self.type_a_emb(
                torch.tensor(AGENT_TYPE.index('seed'), device=device).long())
            shape_emb[is_invalid, (self.num_historical_steps - 1) // self.shift + t] = self.shape_emb(
            ...
            if num_new_agents > 0:
                self.log(t, next_pos_seed, ego_pos_t_1, next_head_seed, ego_head_t_1, next_shape_seed,
                         next_type_idx_seed)
        ...
        for i in range(len(next_token_idx_list)):
            next_token_idx_list[i] = torch.cat([next_token_idx_list[i],
                                                torch.zeros(num_agent - next_token_idx_list[i].shape[0], 1,
                                                            device=device) - 1],
                                               dim=0).long()  # -1: invalid motion token
            next_state_idx_list[i] = torch.cat(
                [next_state_idx_list[i], torch.zeros(num_agent - next_state_idx_list[i].shape[0], 1, device=device)],
                dim=0).long()  # 0: invalid state token
        ...
        pred_traj = torch.cat(
            [torch.zeros(num_agent, self.num_historical_steps, *(pred_traj.shape[2:]), device=pred_traj.device),
             pred_traj], dim=1)
        pred_head = torch.cat(
            [torch.zeros(num_agent, self.num_historical_steps, *(pred_head.shape[2:]), device=pred_head.device),
             pred_head], dim=1)
        pred_state = torch.cat(
            [torch.zeros(num_agent, self.num_historical_steps, *(pred_state.shape[2:]), device=pred_state.device),
             pred_state], dim=1)
        ...
        pred_state[:num_init_agent, 1: self.num_historical_steps] = data['agent']['state_idx'][filter_mask, :(
                                                                                                                         self.num_historical_steps - 1) // self.shift].repeat_interleave(
        ...
        historical_token_traj_all = torch.gather(agent_token_traj_all, 1,
        ...
        historical_token_traj_all = torch.bmm(historical_token_traj_all.view(-1, 4, 2),
                                              rot_mat[:, None, None, ...].repeat(1, (
                                                          self.num_historical_steps - 1) // self.shift, self.shift + 1,
                                                                                 1, 1).view(
                                                  -1, 2, 2)).view(num_init_agent,
        ...
        pred_traj[:num_init_agent, 1: self.num_historical_steps] = historical_token_traj_all[:, :, 1:,
                                                                   ...].clone().mean(dim=3).reshape(num_init_agent, -1,
                                                                                                    2)
        diff_xy = historical_token_traj_all[..., 1:, 0, :] - historical_token_traj_all[..., 1:, 3, :]
        pred_head[:num_init_agent, 1: self.num_historical_steps] = torch.arctan2(diff_xy[..., 1],
                                                                                 diff_xy[..., 0]).reshape(
            num_init_agent, -1)
        ...
        # sanity check
        assert torch.all(
            pos_a[next_state_idx == self.invalid_state] == 0), f'Invalid step should have all zeros position!'
