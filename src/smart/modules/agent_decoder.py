...
class SMARTAgentDecoder(nn.Module):
    ...
    def forward(
        ...
        if self.learn_init:
            ...
            abs_time = tokenized_agent["abs_time"][:, 1:current_step].clone()
            batch = tokenized_agent['batch']

            head_a = gt_head[:,1 :current_step]
            mask = gt_valid[:, 1:current_step]
            pos_a = gt_pos[:, 1:current_step]
            sampled_idx = gt_sampled_idx[:, 1:current_step]

            token_mask = tokenized_agent["token_mask"][:, 1:current_step].clone()
