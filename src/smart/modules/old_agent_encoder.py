...
from .build_edge import radiusGraphNearest, radiusGraphNearest_inv
...
class SMARTAgentDecoder(nn.Module):
    ...
    def forward(
        ...
        feat_a = self.agent_token_embedding(
            agent_token_index=tokenized_agent["sampled_idx"][:, :n_step].long(),  # [n_ag, n_step]
        ...
        mask=mask[:len(tokenized_agent["sampled_idx"])]
        ...
        if self.pred_light:
            ...
            light_mask = torch.ones_like(light_idx).to(torch.bool)#tokenized_agent["light_valid_mask"]
        ...
        if "gt_pos_raw" not in tokenized_agent.keys():
            ...
            return {
                ...
                "feat_a": feat_a[:, 1:-1],
                "agent_q": next_token_logits[:, 1:,None],
                "light_q": light_q[:,:,None],
            }
        else:
            if next_light_logits is not None:
                light_q = next_light_logits[:, 1:]
            ...
            return {
                ...
                "feat_a": feat_a[:, 1:-1],
                "agent_q": next_token_logits[:, 1:],
                "light_q": light_q,
            }

    def autoregressive_light_prediction(self, initial_token, tokenized_agent, max_len, num_graphs):
        ...
        for t in range(current_len, max_len + current_len):
            ...
            if t == current_len:
                ...
                edge_index_lg2lg_T, r_lg2lg_T = self.build_interaction_edge(
                    pos_a=pos_lg[:, None].repeat(1, t, 1),  # [n_agent, hist_step, 2]
                    head_a=head_lg[:, None].repeat(1, t),  # [n_agent, hist_step]
                    head_vector_a=head_vector_lg[:, None].repeat(1, t, 1),  # [n_agent, hist_step, 2]
                    batch_s=batch_lg,  # [n_agent*hist_step]
    ...
    def inference(
        ...
        pred_valid = tokenized_agent["valid_mask"][:len(tokenized_agent["gt_idx"])].clone()
