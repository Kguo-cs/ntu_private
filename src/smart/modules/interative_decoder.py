...
class InterativeDecoder(nn.Module):
    ...
    def forward(self,all_features,map_feature,train_mask ):
        ...
        mask_s = mask_a.flatten(0, 1)#.transpose(0, 1)
        pos_s = pos_a.flatten(0, 1)#.transpose(0, 1)
        head_s = head_a.flatten(0, 1)#.transpose(0, 1)
        head_vector_s = head_vector_a.flatten(0, 1)#
        batch_s = batch_s.flatten(0, 1)#.transpose(0, 1)
        batch_s_repeat = batch_s_repeat.flatten(0, 1)#.transpose(0, 1)

        batch_pl = map_feature["batch"] #batch_pl#.transpose(0, 1).flatten(0, 1) #map_feature["batch"]#batch_pl.flatten(0, 1) #
