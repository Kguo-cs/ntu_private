...
class InterativeDecoder(nn.Module):
    ...
    def predict_agent(self, feat_a, feat_map,
        ...
        if self.edge_encoder.use_roformer:
            ...
            feat_a_t = self.a_t_roformer.temporal_embed(feat_a_t.transpose(0, 1), None, None, n_step, n_current,
                                                        inference_mask)
    ...
    def forward(self, all_features, feat_a, token_embedding, map_feature, agent_train_mask, n_current, pred_mask,
                counter_feat_a=None):
        ...
        if not self.discriminator and self.pred_exit and pred_mask is not None:
            next_token_logits[pred_mask[None].repeat(inference_mask.shape[1], 1)[
                inference_mask.transpose(0, 1)], -1] = -torch.inf  # t,a

        return next_token_logits, feat_a_value, rewards, weight, (edge_index_a2a, r_a2a, relative_pos)

    def get_reward(self, weight, edge_value, end_index, valid_number, mask_ta_flatten, train_repeat_mask, n_step,
                   n_agent):
