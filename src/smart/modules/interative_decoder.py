...
class InterativeDecoder(nn.Module):
    ...
    def forward(self,all_features,map_feature,train_mask ):
        ...
        all_features=[feat.flatten(0, 1) for feat in all_features ]#.transpose(0, 1)
        feat_a,pos_s, head_s, head_vector_s,mask_s, batch_s,batch_s_repeat,batch_pl=all_features

        batch_pl = batch_pl #map_feature["batch"]
        pos_pl = map_feature["position"][:,None].repeat( 1,n_step,1).flatten(0, 1)
        orient_pl = map_feature["orientation"][:,None].repeat(1,n_step).flatten(0, 1)
        feat_map = map_feature["pt_token"].unsqueeze(1).expand( -1,n_step, -1).flatten(0, 1)
        ...
        feat_a = feat_a.view(n_agent,-1,  self.hidden_dim)#.transpose(0, 1)
#tensor(7.992, device='cuda:0', grad_fn=<MaxBackward1>)
#tensor(-0.050, device='cuda:0', grad_fn=<MeanBackward0>)
        proposal=None
