...
class InitDecoder(nn.Module):
    ...
    def pred_entry(self,attr_all_feature,all_pos, all_head,agent_n,n_current=0,tgt_mask=None):
        ...
        entry_logit = self.entry_head_decoder(attr_feature[:,pos_mask])
    ...
    def forward(self,map_feature,ego_feature, tokenized_agent):
        ...
        if self.training:
            initial_pos_token = tokenized_agent["initial_pos_token"]
            initial_offset_xyh = tokenized_agent["initial_offset_xyh"]
            initial_heading_token = tokenized_agent["initial_heading_token"]
            initial_shape= tokenized_agent["initial_shape"]
            initial_type=tokenized_agent["initial_type"]
            ...
            initial_pos= tokenized_agent["initial_pos"][non_ego_mask]
            initial_heading=tokenized_agent["initial_heading"][non_ego_mask]
            batch=tokenized_agent["batch"][non_ego_mask]
