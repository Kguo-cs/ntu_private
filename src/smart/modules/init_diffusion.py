        ...
        for t in range(reverse_steps, 0, -stride):
            ...
            if guid_method == 'ECMR':
                with torch.no_grad():
                    ...
                    g_init_theta = self.net(copy.deepcopy(x_init_t), beta_emb, data, scene_enc, num_samples=num_samples,
                                            eval_mask=eval_mask, clean_data=clean_data)
                ...
                with torch.inference_mode(False):
                    ...
                    temp_x_0_unnor = temp_x_0 * (latent_std.unsqueeze(0).unsqueeze(0) + 0.1) + latent_mean.unsqueeze(
                        0).unsqueeze(0)
                    rec_traj = torch.matmul(temp_x_0_unnor.permute(1, 0, 2),
                                            V_k.unsqueeze(0).repeat(num_samples, 1, 1)) + s_mean.unsqueeze(0)
