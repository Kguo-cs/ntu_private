...
class SMARTAgentDecoder(nn.Module):
    def __init__(
        ...
        if self.pred_agent:
            ...
            self.t_attn_layers = RoFormerBlock(hidden_dim=hidden_dim, num_heads=num_heads, dropout=self.light_dropout)
            self.pt2a_attn_layers = RoFormerBlock(hidden_dim=hidden_dim, num_heads=num_heads,
                                                  dropout=self.light_dropout)
            self.a2a_attn_layers = RoFormerBlock(hidden_dim=hidden_dim, num_heads=num_heads, dropout=self.light_dropout)
    ...
    def spatial_embed(self, feature, network, lengths, sinusoidal_poshead):
        ...
        src_key_padding_mask = (padded_feature.abs().sum(dim=-1) == 0)  # sum along embed dim → 0 means PAD

        padded_feature = network(padded_feature, src_key_padding_mask[:, None, :, None],
                                 sinusoidal_pos=sinusoidal_poshead)
    ...
    def predict_light_roformer(self, light_idx, sinusoidal_poshead, lengths, n_current=0):
        ...
        feat_lg = self.temporal_embed(feat_lg, self.temp_rel, n_step, n_current, self.light_hist)
        ...
        feat_lg = self.spatial_embed(feat_lg, self.spatial_rel, lengths, (sin, cos))

        next_light_logits = self.light_token_predict_head(feat_lg)

        return feat_lg, next_light_logits

    def predict_agent(self, tokenized_agent, map_feature):
    ...
    def predict_agent_roformer(self, tokenized_agent, map_feature, n_current=0):
        ...
        feat_a = self.temporal_embed(feat_a, self.t_attn_layers, n_step, n_current, self.agent_hist, mask)

        feat_a = self.spatial_embed(feat_a, self.a2a_attn_layers, )
    ...
    def forward(
        ...
        if self.pred_light:
            ...
            # feat_lg2, next_light_logits2 = self.predict_light_roformer(noised_light_idx[:,:3], pos_lg, head_lg, batch_lg)
            #
