...
def compare_nested(
    obj1: Any,
    obj2: Any,
    name1: str = "new_tokenize",
    name2: str = "cached",
    path: str = "",
    atol: float = 1e-6,
    rtol: float = 1e-5,
):
    """递归比较 dict/list/tensor，并打印所有差异。"""
    ...
    if isinstance(obj1, dict) and isinstance(obj2, dict):
        ...
        for key in sorted(keys1 - keys2):
            print(f"[ONLY {name1}] {path}{key}")

        for key in sorted(keys2 - keys1):
            print(f"[ONLY {name2}] {path}{key}")
    ...
    if torch.is_tensor(obj1) and torch.is_tensor(obj2):
        ...
        if obj1.shape != obj2.shape:
            print(
                f"[SHAPE] {tensor_path}: "
                f"{name1}={tuple(obj1.shape)}, "
                f"{name2}={tuple(obj2.shape)}"
            )
            return
        ...
        # bool、integer、index 等字段必须精确一致
        if not (x.is_floating_point() or x.is_complex()):
            mismatch = x != y
            num_mismatch = int(mismatch.sum())

            if num_mismatch > 0:
                first_idx = mismatch.nonzero(as_tuple=False)[0].tolist()

                print(
                    f"[VALUE] {tensor_path}: "
                    f"{num_mismatch}/{x.numel()} different, "
                    f"first_index={first_idx}, "
                    f"{name1}={x[tuple(first_idx)].item()}, "
                    f"{name2}={y[tuple(first_idx)].item()}"
                )
            return
        ...
        num_mismatch = int((~close).sum())

        if num_mismatch > 0:
            finite = torch.isfinite(x) & torch.isfinite(y)

            if finite.any():
                abs_diff = (x[finite] - y[finite]).abs()
            ...
            first_idx = (~close).nonzero(as_tuple=False)[0].tolist()
    ...
    if type(obj1) is not type(obj2):
        print(
            f"[TYPE] {path[:-1]}: "
            f"{name1}={type(obj1).__name__}, "
            f"{name2}={type(obj2).__name__}"
        )
        return
