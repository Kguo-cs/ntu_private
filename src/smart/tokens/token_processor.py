...
class TokenProcessor(torch.nn.Module):
    ...
    def process_data(self, data):
        ...
        if len(map) == 0:
            ...
            if "token_idx" in map.keys():
                ...
                if self.noise:
                    topk_indices = torch.argsort(dist, dim=1)[:, :8]
                    sample_topk = torch.randint(0, topk_indices.shape[-1], size=(topk_indices.shape[0], 1),
        ...
        tokenized_agent = self.tokenize_agent(data, False)
        ...
        #     else:
        #         tokenized_agent["token_mask"] = torch.cat([agent["valid_mask"][:, :1], agent["valid_mask"][:, :-1]],
        #                                                   dim=-1)
        #
    ...
    def traj_to_idx(self, sampled_traj, token_agent_shape, token_traj):
        ...
        contour_local = cal_polygon_contour(sampled_traj[..., :2], sampled_traj[..., 2],
                                            token_agent_shape[:, None, None])
