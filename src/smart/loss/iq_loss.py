...
def get_proposal_loss(proposal,tokenized_agent,start_step):
    ...
    token_agent_shape = tokenized_agent["token_agent_shape"][:len(proposal), None, None, None]#[train_mask]
    sampled_pos = tokenized_agent["sampled_pos"][:len(proposal), start_step:-1]#[train_mask]
    sampled_heading = tokenized_agent["sampled_heading"][:len(proposal), start_step:-1]#[train_mask]
    target_global_traj = tokenized_agent["target_global_traj"][:len(proposal), start_step:-1,:proposal.shape[3]]#[train_mask]
    target_mask = tokenized_agent["target_mask"][:len(proposal), start_step:-1, None,:proposal.shape[3]]#[train_mask]
