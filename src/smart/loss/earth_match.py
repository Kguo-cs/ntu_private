...
def get_matching_loss(
    ...
    t_eps=0.05,w_pos=0.1, w_heading=0.5, w_shape=1,w_vel=0.2
    ):
    ...
    if use_col and x_pred:
        ...
        col_loss=multi_circle_collision_loss_mem_efficient(fake_state[:,:2], torch.atan2(fake_state[:,3],fake_state[:,2]), fake_state[:,4],fake_state[:,5],tokenized_agent["nonego_batch"][:len(fake_state)] )[0].mean()
    else:
