...
def matching_loss(
    ...
    pos_loss=F.huber_loss(fake_pos, real_pos)
    ...
    heading_loss= F.huber_loss(fake_heading, real_heading)

    # Shape: L1
    shape_loss = F.huber_loss(fake_shape[:,:2], real_shape[:,:2])
    ...
    vel_loss = F.huber_loss(fake_shape[:,2:][cluster_valid_mask], real_shape[:,2:][cluster_valid_mask])
