...
class RoFormerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if self.headwise_attn_output_gate:
            ...
            query_states = query_states.view(bsz, q_len, self.num_attention_heads, -1)
            query_states, gate_score = torch.split(query_states, [self.attention_head_size, self.attention_head_size],
                                                   dim=-1)
        ...
        if self.caching_len:
            ...
            value_layer = self.cached_v = torch.cat((self.cached_v, value_layer), dim=2)[
                :, :, -self.caching_len * n_agent:]
        elif self.caching:
...
class RoFormerSinusoidalPositionalEmbedding(nn.Module):
    ...
    def __init__(self, hidden_dim, num_heads):
        ...
        freqs_x, freqs_y, freqs_z, freqs_t = self.init_random_2d_freqs(dim=hidden_dim // num_heads, num_heads=num_heads,
                                                                       theta=1000)
