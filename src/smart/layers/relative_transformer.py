...
class RoFormerSinusoidalPositionalEmbedding(nn.Module):
    ...
    def forward(self,positions=None,heading=None,time=None):
        ...
        #
        #     theta_pos=torch.cat([theta_pos,theta_time.repeat_interleave(positions.shape[0],dim=0)],dim=-2)
        #
        ...
        theta = freqs_xyh +freqs_t
        #
        sinusoidal_pos = torch.cat([torch.sin(theta), torch.cos(theta)], dim=-1)
...
class RoFormerBlock(nn.Module):
    ...
    def temporal_embed(self,feature, pos, heading, n_step, n_current,  mask,n_agent=1):
        ...
        #[:,None].repeat(1,n_agent).flatten(0,1)[None, :mask.shape[1], None][:,-pos.shape[1]:]
