...
class IQ_SoftQ(LightningModule):
    ...
    def iq_update(self, tokenized_map, tokenized_agent):
        ...
        if self.reward_w==0:
            ...
            critic_loss=-expert_reward.mean()+agent_reward.mean()#((-expert_reward/alpha).exp()+1).log().mean()+((agent_reward/alpha).exp()+1).log().mean() #((-expert_reward/alpha).exp()+1).log().mean()
            # alpha=0.1
            #
