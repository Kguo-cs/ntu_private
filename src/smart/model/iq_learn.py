...
class IQ_SoftQ(LightningModule):
    ...
    def get_QV(self, tokenized_map, tokenized_agent, key='expert'):
        ...
        if div=="kl":
            ...
            reward=torch.clamp(reward,max=alpha*1e3,min=alpha*1e-3)
           ...
           # reward_loss= reward_loss.detach()*reward
            ...
            alpha=1
            reward=torch.clamp(reward,max=alpha*(np.log(1/2+1e3)),min=alpha*(np.log(1/2+1e-3)))
            ...
            alpha = 1
            reward=torch.clamp_min(reward,min=0)
