...
class IQ_SoftQ(LightningModule):
    ...
    def get_QV(self, tokenized_map, tokenized_agent, train_mask, key='expert'):
        ...
        if self.token_processor.pred_exit and key=='expert':
            ...
            action_nll=action_nll+0.01*exit_nll
    ...
    def get_reward(self, tokenized_agent, agent_log_prob, agent_pi, key, train_mask=None, expert_disc_val=0,
        ...
        if key=="agent":
            batch_rewards=torch.zeros([train_mask.shape[1],train_mask.shape[0]],device=train_mask.device)
            mask_s=tokenized_agent["valid_mask"][:,1+self.start_step:].transpose(0,1)
            batch_rewards[mask_s]=rewards
