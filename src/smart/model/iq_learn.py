...
class IQ_SoftQ(LightningModule):
    ...
    def get_network_QV(self,network,tokenized_map, tokenized_agent,action,key):
        ...
        if self.output_gmm:
            ...
            logpi= torch.log(pi).clamp_min(min=np.log(1e-10))#+ 1e-10

            # log_pi_stack=torch.log(torch.softmax(pred["q_value"][:, :-1]/ self.alpha, dim=-1) ).clamp_min(min=np.log(1e-10))#+ 1e-10
            #
