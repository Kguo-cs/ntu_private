...
def plot_figure(P,G_masked,alpha_pred=0.35,lw_pred=0.8, lw_ref=2.0,):
...
def plot_bird_from_tensors(pred_traj, sampled_pos, gt_pos_raw, gt_valid_raw,
    ...
    # speed=np.linalg.norm(P[:,:,1:]-P[:,:,:-1], axis=-1)
    #
    # gt_speed=np.linalg.norm(G_masked[:,1:]-G_masked[:,:-1], axis=-1)
    #
    # print(np.max(speed[~np.isnan(speed)]))
    # print(np.max(gt_speed[~np.isnan(gt_speed)]))

    #plot_figure(P,G_masked)
    ...
    def update(frame_idx):
        ...
        gt_n=(~np.isnan(G_np[:,frame_idx])).sum()
        pred_n=(~np.isnan(P_np[:,0,frame_idx])).sum()
