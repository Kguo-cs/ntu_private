...
def _robust_component_loss(fake, real, beta=0.2, use_huber=True):
    if use_huber:
        return F.smooth_l1_loss(fake, real, reduction="none", beta=beta).mean(-1)#2.0 * beta*
...
def matching_loss(
    ...
    if fake_state.shape[-1]<16:
        ...
        shape_loss = _robust_component_loss(fake_shape, real_shape, beta=1, use_huber=False)
        vel_loss = _robust_component_loss(fake_vel, real_vel, beta=1, use_huber=False)
