...
class ObservationNormalizer:
    ...
    def __call__(self, data):
        ...
        for k, v in self._normalization_dict.items():
            ...
            try:
                norm_data[k] = (data[k] - torch.tensor(v["mean"]).to(data[k].device)) / torch.tensor(v["std"]).to(
                    data[k].device)
    ...
    def to_dict(self):
        return {k: {kk: vv.detach().cpu().numpy().tolist() for kk, vv in v.items()} for k, v in
                self._normalization_dict.items()}