...
class ScaleFlow(nn.Module):
    ...
    def get_loss(self,
        ...
        if self.model.use_cfg_cond:
            tokenized_agent["cfg"]= torch.ones(num_graphs,device=agent_batch.device)*3#sample_cfg_scale(num_graphs,device=z.device)#t
    ...
    def sample(self,tokenized_agent,initial_map_feature,eval_mask,infer_steps=20,num_samples=1,noise_level=None):
        ...
        if self.model.use_cfg_cond:
            tokenized_agent["cfg"]=torch.ones(num_graphs,device=agent_batch.device)*3
