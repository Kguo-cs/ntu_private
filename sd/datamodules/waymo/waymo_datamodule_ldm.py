...
class WaymoDataModuleLDM(pl.LightningDataModule):
    ...
    def _initialize_pyg_dset(self, mode, num_samples, batch_size, conditioning_path=None,
        ...
        for i in range(num_samples):
            ...
            if mode == 'initial_scene':
                ...
                d['lane'].x = torch.empty((num_lanes, 40))
                d['agent'].x = torch.empty((num_agents, 8))
                ...
                d['lane'].x = from_numpy(road_points).to(
                    torch.float) * 32  # torch.empty((num_lanes, self.cfg_model.lane_latent_dim))
    ...
    def setup(self, stage):
        ...
        # self.val_dataset = WaymoDatasetLDM(self.cfg_dataset, split_name='train')
